        ...
        <div className={`h-auto sm:max-w-sm sm:m-2 rounded-xl bg-gray-900 border-gray-800 transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
            <div className="flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4">
                            ...
                            if (isMessage) {
                                ...
                                return (
                                    <div key={msg.id} className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                                        ...
                                        <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-md text-sm mt-5 leading-relaxed ${isUser ? 'bg-linear-to-r  from-indigo-500 to-indigo-600 text-white rounded-tr-none' : 'rounded-tl-none bg-gray-800 text-gray-100'} `}>
                                            {msg.content}
                                        </div>
                    ...
                    {/* Loading Animation While ai generating response */}
                    {
                        isGenerating && (
